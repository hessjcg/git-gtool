...
func checkStatusChecks(ctx context.Context, client *github.Client, org string, repo string, base string, activePr *github.PullRequest) error {
	...
	checks, err := checkCheckRuns(ctx, client, org, repo, activePr, count)
	if err != nil {
		return err
	}
	...
	for context, conclusion := range checkResults {
		log.Printf(" check %v %v", context, conclusion)
		if conclusion != "success" {
			failedCheck = true
		}
	}
	if failedCheck || len(checkResults) == 0 {
		return fmt.Errorf("the PR is missing required checks: %v/%v %v", org, repo, activePr.GetNumber())
	}
	return nil
}
...
func approvePr(ctx context.Context, client *github.Client, org string, repo string, activePr *github.PullRequest) error {
	// Approve PR
	log.Printf("Ensuring PR #%4d is approved.", activePr.GetNumber())
...
func checkCheckRuns(ctx context.Context, client *github.Client, org string, repo string, activePr *github.PullRequest, count int) ([]runResult, error) {
	var results []runResult
